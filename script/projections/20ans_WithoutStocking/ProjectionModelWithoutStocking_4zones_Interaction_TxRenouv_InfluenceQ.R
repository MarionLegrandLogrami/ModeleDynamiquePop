...
# Modèle avec les 4 zones / interaction sauvage-élevage / influence Q sur survie et répartition des adultes 
#
...
bugs_s_juv2ad=read.coda("simulation/s_juv2adCODAchain1.txt","simulation/s_juv2adCODAindex.txt") ###prend en compte influence des Q sur la survie
...
for (t in (T-1):(T+3)){
	for (i in 1:iteration){ 
		...
		L_mu_p_alagnon[i,t]<-L_ratio_juv_A[i,t]+bugs_adjust_p_A[i]+beta_Q_L_mont*L_Q_L_mont_cr[t] # beta_Q_L_mont*L_Q_L_mont_cr[t]= influence des Q sur la répartition des adultes
		...
		L_mu_p_langeac[i,t]<-L_ratio_juv_L[i,t]+bugs_adjust_p_L[i]+beta_Q_VB_mont*L_Q_VB_mont_cr[t] # beta_Q_VB_mont*L_Q_VB_mont_cr[t]= influence des Q sur la répartition des adultes
		...
		L_mu_p_poutes[i,t]<-L_ratio_juv_P[i,t]+bugs_adjust_p_P[i]+beta_Q_P_mont*L_Q_P_mont_cr[t] #  beta_Q_P_mont*L_Q_P_mont_cr[t]= influence des Q sur la répartition des adultes
...
moy11_20<-round(mean(data_vichy[(T-9):T]),0) #658
...
mean(upper_30_ratio_vichy) #93.6%
mean(upper_50_ratio_vichy) #86.3%
mean(upper_80_ratio_vichy) #44.9%
...
mean(upper_20_ratio_vichy_10) #93.5%
...
under_10_vichy=array(NA,dim=c(iteration,25))
under_50_vichy=array(NA,dim=c(iteration,25))
under_100_vichy=array(NA,dim=c(iteration,25))
under_250_vichy=array(NA,dim=c(iteration,25))
under_500_vichy=array(NA,dim=c(iteration,25))
...
p_under_10_vichy=rep(NA,25)
p_under_50_vichy=rep(NA,25)
p_under_100_vichy=rep(NA,25)
p_under_250_vichy=rep(NA,25)
p_under_500_vichy=rep(NA,25)
