...
library(boot)
T=39
#Attention commence à l'année 7
bugs_d_moy_vichy=read.coda("dmoytot_VCODAchain1.txt","dmoytot_VCODAindex.txt")
bugs_d_moy_langeac=read.coda("dmoytot_LCODAchain1.txt","dmoytot_LCODAindex.txt")

#Attention commence à l'année 13
bugs_d_moy_poutes=read.coda("dmoytot_PCODAchain1.txt","dmoytot_PCODAindex.txt")#,1,5000)
#Pour ne pas trainer cette difficulté on remet les 0 avant l'année 13
d_moy_poutes=array(0,dim=c(5000,T+20))
for (t in 13:(T+20)){
	for (i in 1:5000){ 
		d_moy_poutes[i,t]=bugs_d_moy_poutes[i,t-12]
	}
}
mean(bugs_d_moy_vichy[,1])
...
#Pour ne pas trainer cette difficulté on remet les 0 avant l'année 7
juv_tot_V=array(0,dim=c(5000,T+20))
juv_tot_L=array(0,dim=c(5000,T+20))
for (t in 7:(T+20)){
	for (i in 1:5000){ 
		juv_tot_V[i,t]=bugs_juv_tot_V[i,t-6]
		juv_tot_L[i,t]=bugs_juv_tot_L[i,t-6]
	}
}

bugs_juv_tot_system=read.coda("simulation/juv_tot_systemCODAchain1.txt","simulation/juv_tot_systemCODAindex.txt")
mean(bugs_juv_tot_system[40])
...
#les juvéniles de Vichy et Langeac sont déjà calculés dans OpenBugs
juv_poutes=array(0,dim=c(5000,T+20))

juv_tot_P=array(0,dim=c(5000,T+20))
...
for (t in (T+1):(T+3)){
	for (i in 1:5000){ 
		juv_poutes[i,t-3]=d_moy_poutes[i,t-3]*S_juv_JP[3,t-3]
		juv_poutes[i,t-4]=d_moy_poutes[i,t-4]*S_juv_JP[3,t-4]
		juv_poutes[i,t-5]=d_moy_poutes[i,t-5]*S_juv_JP[3,t-5]
		
		juv_tot_P[i,t] <- bugs_rho_poutes[i]*(1/3) * juv_poutes[i,t-3] + bugs_rho_poutes[i]*(1/3) * juv_poutes[i,t-4] + bugs_rho_poutes[i]*(1/3) * juv_poutes[i,t-5]
		juv_tot_system[i,t] <- juv_tot_V[i,t]+juv_tot_L[i,t] +juv_tot_P[i,t]
	}
}
...
for (t in (T+4):(T+4)){
	for (i in 1:5000){ 
		...
		juv_tot_P[i,t] <- ((1+bugs_rho_poutes[i])/2)*(1/3) * juv_poutes[i,t-3] + bugs_rho_poutes[i]*(1/3) * juv_poutes[i,t-4] + bugs_rho_poutes[i]*(1/3) * juv_poutes[i,t-5]
		juv_tot_system[i,t] <- juv_tot_V[i,t]+juv_tot_L[i,t] +juv_tot_P[i,t]
	}
}
#Amélioration sur les deux premiers tiers de production
for (t in (T+5):(T+5)){
	for (i in 1:5000){ 
		juv_poutes[i,t-3]=d_moy_poutes[i,t-3]*S_juv_JP[3,t-3]
		juv_poutes[i,t-4]=d_moy_poutes[i,t-4]*S_juv_JP[3,t-4]
		...
		juv_tot_P[i,t] <- ((1+bugs_rho_poutes[i])/2)*(1/3) * juv_poutes[i,t-3] + ((1+bugs_rho_poutes[i])/2)*(1/3) * juv_poutes[i,t-4] + bugs_rho_poutes[i]*(1/3) * juv_poutes[i,t-5]
		juv_tot_system[i,t] <- juv_tot_V[i,t]+juv_tot_L[i,t] +juv_tot_P[i,t]
	}
}	
#Amélioration pour l'ensemble de la production de juvéniles
for (t in (T+6):(T+20)){
	for (i in 1:5000){ 
		juv_poutes[i,t-3]=d_moy_poutes[i,t-3]*S_juv_JP[3,t-3]
		juv_poutes[i,t-4]=d_moy_poutes[i,t-4]*S_juv_JP[3,t-4]
		...
		juv_tot_P[i,t] <- ((1+bugs_rho_poutes[i])/2)*(1/3) * juv_poutes[i,t-3] + ((1+bugs_rho_poutes[i])/2)*(1/3) * juv_poutes[i,t-4] + ((1+bugs_rho_poutes[i])/2)*(1/3) * juv_poutes[i,t-5]
		juv_tot_system[i,t] <- juv_tot_V[i,t]+juv_tot_L[i,t] +juv_tot_P[i,t]
	}
}	
#Vérif qu'on augmente bien le nbr de sat produit en amont de Poutès
mean(juv_tot_P[,43:59])#17 485.82
bugs_juv_tot_P=read.coda("simulation/juv_tot_PCODAchain1.txt","simulation/juv_tot_PCODAindex.txt")
mean(bugs_juv_tot_P[28:44])#6 498.412
mean(juv_tot_system[,43:59])#117 777.5
mean(bugs_juv_tot_system[,37:53])#111 820.3
...
#Les juvéniles calculés sur chaque zone sont directement repris du modèle 
#openbugs pour Vichy et Langeac (avec seulement la modif de l'ajout des 0 avant début données T=7). 
#MAIS on utilise les juvéniles recalculés dans la première partie pour Poutès
for (t in (T+1):(T+20)){
	for (i in 1:5000){ 
		...
		ratio_juv_prod_L[i,t] <- (juv_tot_L[i,t]+juv_tot_P[i,t]) / (juv_tot_V[i,t]+juv_tot_L[i,t]+juv_tot_P[i,t])	
		...
		ratio_juv_prod_P[i,t] <-  juv_tot_P[i,t]/(juv_tot_L[i,t]+juv_tot_P[i,t])
...
# Graph
#------------------
png(filename="C:/Users/utilisateur/workspace/ModeleDynamiquePop/img/Simulation/2014_12_05_Poutes50/poutes_TotalReturns_proj20years.png",width=800,height=800)
...
# Graph
#------------------------
png(filename="C:/Users/utilisateur/workspace/ModeleDynamiquePop/img/Simulation/2014_12_05_Poutes50/poutes_Threshold.png",width=800,height=800)
