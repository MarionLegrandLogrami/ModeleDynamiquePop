...
#========================================================
# Rï¿½cupï¿½ration des paramï¿½tres dans le posterior joint
#========================================================
...
#c'est le niveau ï¿½ atteindre. Ensuite on augmente progressivement level_s_fake jusqu'ï¿½ ce niveau
mean(bugs_level_s) #1.503928
mean(bugs_level_s)/2 #0.7519638

#ATTENTION level_s_fake est en ï¿½chelle log il faut donc prendre l'exponentielle
...
#Les juvï¿½niles t-3 sont maintenant ceux calculï¿½s ï¿½ la fin de la premiï¿½re boucle et non plus ceux issus directement d'openbugs
for (t in (T+5):(T+5)){
	for (i in 1:5000){ 
...
#Les juvï¿½niles t-3 et t-4 sont maintenant ceux calculï¿½s ï¿½ la fin des boucles prï¿½cï¿½dentes et non plus ceux issus directement d'openbugs
for (t in (T+6):(T+6)){
	for (i in 1:5000){ 
...
#Les juvï¿½niles t-3 et t-4 t-5 sont maintenant ceux calculï¿½s ï¿½ la fin des boucles prï¿½cï¿½dentes et non plus ceux issus directement d'openbugs
for (t in (T+7):(T+20)){
	for (i in 1:5000){ 
...
#Attention ï¿½ l'annï¿½e 30 estimation des passages Vichy car annï¿½e jugï¿½e incomplï¿½te
for (t in 1:22){
...
plot(1,1,type="n",axes=FALSE,xlim=c(0.5,T+20+0.5),xlab="Years",ylim=c(0,9000),ylab="Returns Vichy",main=iconv("Projection ï¿½ 20 ans sans repeuplement & Amï¿½lioration de la survie \
						jusqu'ï¿½ 50% du taux de survie initial","UTF8"),cex.lab=1.5)
...
plot(1,1,type="n",axes=FALSE,xlim=c(0.5,20.5),xlab="Years",ylim=c(0,1),ylab=expression(italic(p^threshold)),main=iconv("Projection ï¿½ 20 ans sans repeuplement & Amï¿½lioration de la survie \
 jusqu'ï¿½ 50% du taux de survie initial","UTF8"))
...
#ATTENTION level_s_fake est en ï¿½chelle log il faut donc prendre l'exponentielle
#ATTENTION level_s_fake est en log !! la progression linï¿½aire est visible si on prend l'exponentielle de level_s_fake
...
plot(1,1,type="n",axes=FALSE,xlim=c(0.5,20.5),xlab="Years",ylim=c(0,1),ylab=expression(italic(p^threshold)),main=iconv("Projection ï¿½ 20 ans sans repeuplement & Amï¿½lioration de la survie \
						jusqu'ï¿½ 100% du taux de survie initial","UTF8"))
